# Autotrophic respiration (Ra): 1pctCO2

Line plot: subplots = stage (S0 control, S1 bgc-coupled, S2 fully coupled),
lines = factorial (baseline, no-fire tuned, no-fire untuned — the latter only
appears in the S0 panel, since it has no bgc/cou-ukesm stage).

![Autotrophic respiration (Ra): 1pctCO2 factorials](../../../img/wiemip/benchmarking/1pctco2/lineplot_mra.png)

## Spatial pattern, 1850 vs 2000

Shared color scale per stage figure; NBP uses a diverging scale (blue = net
sink, red = net source), all other variables use a sequential scale.

### S0: control

![Autotrophic respiration (Ra) maps: S0 control](../../../img/wiemip/benchmarking/1pctco2/map_mra_ctrl.png)

### S1: bgc-coupled

![Autotrophic respiration (Ra) maps: S1 bgc-coupled](../../../img/wiemip/benchmarking/1pctco2/map_mra_bgc.png)

### S2: fully coupled (UKESM1-0-LL)

![Autotrophic respiration (Ra) maps: S2 fully coupled](../../../img/wiemip/benchmarking/1pctco2/map_mra_cou-ukesm.png)
